...
- **Stuck detection = current only.** A pump faults (→ switch + cooldown) only when amps leave the band after `STARTUP_GRACE_MS` (under = not pumping/dry/unplugged; over = jammed). The 1/2-float-won't-clear timer (`MAX_CLEAR_HALF_MS`) is **NOT a fault** — with normal current the pump *is* working, so a level that won't drop means inflow ≥ outflow (storm) or slight impairment; we **keep the pump running and only warn** (beacon, `overwhelmed` flag). Rationale: with float-only level sensing you can't tell "losing to inflow" from "stuck", and stopping a working pump mid-flood is the worst outcome. Earlier this was an `F_INEFFECTIVE` fault — deliberately changed; don't reintroduce it. The `MAX_RUN_MS` dry-run cap now only fires while the 1/2 float is already clear (never in a flood).
...
- **Start-rate limit (`MIN_START_INTERVAL_MS`, 180 s):** ≤20 starts/hour per pump (pump-manual limit). Enforced as a **start-to-start** interval, **per pump** (not global, not stop-to-start). A **fault hand-off** (`faultHandoff` flag, set in `raiseFault`) and the **3/4 emergency override** pass `requireRate=false` to **bypass** it — never delay the backup or a flood response. `t_undercurrent_fault_fast_handoff` guards the fast hand-off; `t_start_rate_limit` guards the 180 s gate. (There is intentionally **no max run-time** — the pump has none; the only run cap is the `MAX_RUN_MS` dry-run backstop, which fires only while the 1/2 float is clear.)
- **Daily auto-drain / exercise (`EXERCISE_INTERVAL_MS`, 24 h):** if MIN is wet but no pump has started in 24 h, run one (drains lingering water + keeps pumps from seizing). It's a third demand source (`fHalf || preEmpty || exerciseDue`, with `exerciseDue = fMin && now-lastStartAny ≥ 24 h`); reuses the normal drain-to-MIN path, never runs dry. `t_exercise_autodrain` guards it.
